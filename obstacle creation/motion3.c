...
//  Matches your Verilog‘s `case(address)` slots:
...
// ------------------------------------------------------------------
//  Platform stubs (to satisfy linker)
//  Replace these with real implementations if you have them.
// ------------------------------------------------------------------
...
void read_controller(void) {
    // no-op: keep dino at whatever REG_DINO_Y was last set
}
bool start_pressed(void) {
    return false;  // never restarts
}
...
// ------------------------------------------------------------------
//  Main game loop driving obstacle motion & collision
// ------------------------------------------------------------------
int main(void) {
    // 1) Initialize from PIO (must match your Verilog reset values)
    int dino_x = REG_DINO_X;   // input from controller
